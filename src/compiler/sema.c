...
int sema_analyze_expr(Sema *sema, AstNode *node)
{
    ...
    case AST_EXPR_CALL:
    {
        ...
        if (node->call_expr.func && node->call_expr.func->kind == AST_EXPR_IDENT)
        {
            ...
            bool is_atomic_builtin = bname && (
                !strcmp(bname, "atomic_load") ||
                !strcmp(bname, "atomic_store") ||
                !strcmp(bname, "atomic_cas") ||
                !strcmp(bname, "atomic_fence") ||
                !strcmp(bname, "atomic_exchange") ||
                !strcmp(bname, "atomic_fetch_add") ||
                !strcmp(bname, "atomic_fetch_sub"));
            if (is_atomic_builtin)
            {
                if (node->call_expr.args)
                {
                    for (int i = 0; i < node->call_expr.args->count; i++)
                ...
                if (!strcmp(bname, "atomic_fence"))
                {
                    if (arg_count != 0)
                    {
                        sema_error(sema, node->token, "atomic_fence takes no arguments");
                        return -1;
                    }
                    node->type = NULL;
                    return 0;
                }
                if (!strcmp(bname, "atomic_store"))
                {
                    if (arg_count != 2)
                    {
                        sema_error(sema, node->token, "atomic_store expects 2 arguments (ptr, val)");
                        return -1;
                    }
                    node->type = NULL;
                    return 0;
                }
                if (!strcmp(bname, "atomic_load") || !strcmp(bname, "atomic_exchange") ||
                    !strcmp(bname, "atomic_fetch_add") || !strcmp(bname, "atomic_fetch_sub"))
                {
                    int expected_args = !strcmp(bname, "atomic_load") ? 1 : 2;
                    if (arg_count != expected_args)
                    {
                        sema_error(sema, node->token, "wrong number of arguments for atomic builtin");
                        return -1;
                    }
                    node->type = type_get_primitive(TYPE_I64);
                    return 0;
                }
                if (!strcmp(bname, "atomic_cas"))
                {
                    if (arg_count != 3)
                    {
                        sema_error(sema, node->token, "atomic_cas expects 3 arguments (ptr, expected, desired)");
                        return -1;
                    }
                    node->type = type_get_primitive(TYPE_I64);
                    return 0;
