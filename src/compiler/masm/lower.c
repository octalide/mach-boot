...
static MasmOperand lower_call_with_sret(Masm *masm, MasmSection *text, AstNode *expr, LowerContext *ctx, MasmOperand *provided_sret)
{
    ...
    if (expr->call_expr.func && expr->call_expr.func->kind == AST_EXPR_IDENT)
    {
        ...
        int         nargs = bargs ? bargs->count : 0;

        if (bname && !strcmp(bname, "atomic_load") && nargs == 1)
        {
            MasmOperand ptr = lower_expr(masm, text, bargs->items[0], ctx);
            ptr = ensure_in_reg(text, ptr, bargs->items[0]->type, ctx);
            MasmOperand dst = alloc_vreg(ctx, 8);
            masm_section_append_inst(text, masm_inst_2(MASM_IR_ATOMIC_LOAD, dst, ptr));
            return dst;
        }
        if (bname && !strcmp(bname, "atomic_store") && nargs == 2)
        {
            MasmOperand ptr = lower_expr(masm, text, bargs->items[0], ctx);
            ptr = ensure_in_reg(text, ptr, bargs->items[0]->type, ctx);
            MasmOperand val = lower_expr(masm, text, bargs->items[1], ctx);
            val = ensure_in_reg(text, val, bargs->items[1]->type, ctx);
            masm_section_append_inst(text, masm_inst_2(MASM_IR_ATOMIC_STORE, ptr, val));
            return masm_operand_none();
        }
        if (bname && !strcmp(bname, "atomic_cas") && nargs == 3)
        {
            MasmOperand ptr      = lower_expr(masm, text, bargs->items[0], ctx);
            ptr = ensure_in_reg(text, ptr, bargs->items[0]->type, ctx);
            MasmOperand expected = lower_expr(masm, text, bargs->items[1], ctx);
            expected = ensure_in_reg(text, expected, bargs->items[1]->type, ctx);
            MasmOperand desired  = lower_expr(masm, text, bargs->items[2], ctx);
            desired = ensure_in_reg(text, desired, bargs->items[2]->type, ctx);
            MasmOperand dst = alloc_vreg(ctx, 8);
            masm_section_append_inst(text, masm_inst_4(MASM_IR_CMPXCHG, dst, ptr, expected, desired));
            return dst;
        }
        if (bname && !strcmp(bname, "atomic_fence") && nargs == 0)
        {
            masm_section_append_inst(text, masm_inst_0(MASM_IR_FENCE));
            return masm_operand_none();
        }
        if (bname && !strcmp(bname, "atomic_exchange") && nargs == 2)
        {
            MasmOperand ptr = lower_expr(masm, text, bargs->items[0], ctx);
            ptr = ensure_in_reg(text, ptr, bargs->items[0]->type, ctx);
            MasmOperand val = lower_expr(masm, text, bargs->items[1], ctx);
            val = ensure_in_reg(text, val, bargs->items[1]->type, ctx);
            MasmOperand dst = alloc_vreg(ctx, 8);
            masm_section_append_inst(text, masm_inst_4(MASM_IR_ATOMIC_RMW, dst, ptr, val, masm_operand_imm(MASM_IR_RMW_XCHG)));
            return dst;
        }
        if (bname && !strcmp(bname, "atomic_fetch_add") && nargs == 2)
        {
            MasmOperand ptr = lower_expr(masm, text, bargs->items[0], ctx);
            ptr = ensure_in_reg(text, ptr, bargs->items[0]->type, ctx);
            MasmOperand val = lower_expr(masm, text, bargs->items[1], ctx);
            val = ensure_in_reg(text, val, bargs->items[1]->type, ctx);
            MasmOperand dst = alloc_vreg(ctx, 8);
            masm_section_append_inst(text, masm_inst_4(MASM_IR_ATOMIC_RMW, dst, ptr, val, masm_operand_imm(MASM_IR_RMW_ADD)));
            return dst;
        }
        if (bname && !strcmp(bname, "atomic_fetch_sub") && nargs == 2)
        {
            MasmOperand ptr = lower_expr(masm, text, bargs->items[0], ctx);
            ptr = ensure_in_reg(text, ptr, bargs->items[0]->type, ctx);
            MasmOperand val = lower_expr(masm, text, bargs->items[1], ctx);
            val = ensure_in_reg(text, val, bargs->items[1]->type, ctx);
            MasmOperand dst = alloc_vreg(ctx, 8);
            masm_section_append_inst(text, masm_inst_4(MASM_IR_ATOMIC_RMW, dst, ptr, val, masm_operand_imm(MASM_IR_RMW_SUB)));
            return dst;
        }
    }
